...
void showRelasiParentToChild(List_peserta L, List_relasi &L3){
    address_relasi P = first3(L3);
    address_peserta Q = first(L);
    int i = 1;
    if ((first3(L3) !=NULL) || ((first(L)!=NULL) && (last(L)!=NULL))){ //not isEmpty
        while (Q!=NULL){
            while (P!=NULL){
                if (info(parent(P)).nim == info(Q).nim){
...
void PesertaLombaTertentu(List_relasi &L3, string cariLomba)
{
    ...
    while (Q !=NULL)
    {
        ...
        {
            cout << "Peserta ke- "<<i<<" : (" << info(parent(Q)).nim <<" )"<< info(parent(Q)).nama << endl;
            i = i+1;
        }
        Q = next(Q);
    }
